...
public class Polynomial implements Comparable<Polynomial> {
	...
	public int compareTo(Polynomial o) {
		...
		if(this.coefficients[this.getDegree()] > o.coefficients[o.getDegree()]) {
			return 1;
		}
		else if (this.coefficients[this.getDegree()] < coefficients[o.getDegree()]) {
			return -1;
		}
		
		
		return coeffComp(this.getDegree(), o); // STUB
	}
	
	public int coeffComp(int deg,Polynomial p) {
		if(this.coefficients[deg] > p.coefficients[deg]) {
			return 1;
		}
		if(this.coefficients[deg] < p.coefficients[deg]) {
			return -1;
		}
		if(deg==0 && this.coefficients[deg] == p.coefficients[deg]) {
			return 0;
		}
		return coeffComp(deg-1, p);
	}
